...
- `src/agbot_vision_nav/row_exit_detector.py` — detects end-of-row from the mask (corridor widening to open field, or blocked-ahead wall). Open fires when the `exit_open_rows_required` (2) FARTHEST scan rows are wide — the far rows widen first, so this fires before the robot reaches the last plants (waiting for all rows overshot the row end by 1.5–2.5 m with the low front camera). Per-signature debounce (`exit_detect_frames` 5 open, `blocked_detect_frames` 8 blocked — leaves brushing the lens must not trigger a back-out) and per-signature arming: open after `min_in_row_distance` m, blocked already after `blocked_arming_distance` m (0.3) so mid-row obstacles near the entrance are still caught.
