...
- `scripts/operator_panel.py` — PyQt panel: start frame source (cameras, or Gazebo when `simulation` is ticked) / start mission with form fields / **pause + resume** / stop. `rosrun agbot_vision_nav operator_panel.py`. **Recommended sim workflow: run Gazebo in its own terminal** (`roslaunch agbot_bringup agbot_gazebo.launch`) and use the panel only for the mission — Gazebo's output otherwise buries the vision-nav startup config block, which is the part worth reading. The `simulation` tick is what puts `sim:=true` on the vision-nav launch; that is independent of who started the simulator, so tick it either way and just don't press the frame-source button when Gazebo is already up.
